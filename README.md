...
> **2026-09-16 补充：CI 产物 60~90 秒崩溃的根因已查明并修复，修复后的 CI 产物
> 已在真机复验通过（`fastboot boot` 连续在线 420 秒无 oops、设备侧自检 17 项全过、
> root 正常）。详见下方「本次事故」。**
...
- **状态**：修复后的配置解析结果与真机验证过的基线 `.config` 逐字节一致（CI 侧只差
  编译器版本串），产物镜像同样大小（29,634,576 B）；修复后的 **CI 产物**已真机复验
  （2026-09-16，CI run `35011287237` 的产物 `fastboot boot`）：
  连续在线 420 秒、dmesg 里 0 条 `Unable to handle`、`su -c id` 返回
  `uid=0(root) … context=u:r:ksu:s0`、`scripts/verify-on-device.sh` **17 项 OK / 0 失败**
  （对照：修复前的 CI 产物在同一台机器上 54.6 秒必崩）。
